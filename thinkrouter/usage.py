...
class UsageSummary:
    ...
    def __str__(self) -> str:
        ...
        for tier in Tier:
            ...
            pct   = (count / self.total_calls * 100) if self.total_calls > 0 else 0.0
            lines.append(
...
class UsageTracker:
    ...
    ----------
    max_records : int
        Maximum individual call records retained in memory.
        Set to 0 to track aggregates only (no per-call records).
    """
    ...
    def record(
        ...
        """
        Record a single routing decision and return the created CallRecord.
